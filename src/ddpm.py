...
class DDPMSampler:
    def __init__(self, generator: torch.Generator, n_training_steps=1000, 
    ...
    def set_inference_steps(self, n_inference_steps=50):
        ...
        """
            self.n_training_steps: 1000, self.n_inference_steps: 50, step_ratio: 20
            timesteps: [980, 960, 940, ..., 0]
    ...
    def _get_variance(self, timestep: int) -> int:
        ...
        # Compute the variance using the formule (7) of the DDPM paper.
    ...
    def step(self, timestep: int, latents: torch.Tensor, model_output: torch.Tensor):
        ...
        # 2. Compute the predicted original sample using the formule (15) of the DDPM paper. 
        # "predicted x_0" of formula (15) from https://arxiv.org/pdf/2006.11239.pdf
        pred_original_sample = (latents - current_beta_t ** 0.5 * model_output) / current_alpha_t ** 0.5  

        # 4. Compute the coefficients for pred_original_sample and current sample x_t
        # See formula (7) from https://arxiv.org/pdf/2006.11239.pdf
        pred_original_sample_coeff = (alpha_prod_t_prev ** 0.5 * current_beta_t) / beta_prod_t
        current_sample_coeff = current_alpha_t ** 0.5 * beta_prod_t_prev / beta_prod_t 

        # 5. Compute the predicted previous sample mean 
