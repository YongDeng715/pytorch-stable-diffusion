...
class VAE_AttentionBlock(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        # x: (batch_size, feature_channels, H, W)
...
class VAE_Decoder(nn.Sequential):
    def __init__(self):
        super().__init__(
            # (N, 4, H/8, W/8) -> (N, 4, H/8, W/8) -> (N, 512, H/8, W/8)  
            ...
            # (N, 512, H/4, W/4) -> (N, 512, H/2, W/2) -> (N, 256, H/2, W/2)           
            nn.Upsample(scale_factor=2),
            nn.Conv2d(512, 512, kernel_size=3, padding=1),
            ...
            # (N, 256, H/2, W/2) -> (N, 256, H, W) -> (N, 3, H, W)
            nn.Upsample(scale_factor=2),
    ...
    def forward(self, x: torch.Tensor):
        # x: (batch_size, 4, H /8, W / 8) -> (batch_size, 3, H, W)
